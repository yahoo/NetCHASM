...
#include "TestHMTCPConnectionHandler1.h"
...
-   name: config2.parse1.healthmon.net\n\
    ...
    lookup4.setPlugin(HM_DNS_PLUGIN_NONE);
    HMDNSLookup lookup6(HM_DNS_TYPE_LOOKUP, true);
    lookup6.setPlugin(HM_DNS_PLUGIN_NONE);
    addresses.insert(address);
    dnsCache.insertDNSEntry(host1, lookup4, 10000, 10000);
    dnsCache.insertDNSEntry(host1, lookup6, 10000, 10000);
    dnsCache.insertDNSEntry(host2, lookup4, 10000, 10000);
    dnsCache.insertDNSEntry(host2, lookup6, 10000, 10000);
    dnsCache.updateDNSEntry(host1, lookup4, addresses);
    addresses.clear();
    addresses.insert(address3);
    dnsCache.updateDNSEntry(host1, lookup6, addresses);
    addresses.clear();
    addresses.insert(address2);
    dnsCache.updateDNSEntry(host2, lookup6, addresses);
...
void TESTNAME::test_cmdlstnr1()
{
    ...
    HMIPAddress source;
    const HMConnectionCheck check(port, server, HM_REMOTE_SHARED_CHECK_TCP, source, 0);
    const HMConnectionCheck checkl(socketPath, HM_REMOTE_SHARED_CHECK_LINUX);
    HMConnectionHandler handler(5, tvc);
...
void TESTNAME::test_cmdlstnr7()
{
    ...
    lfb = data->getLFB();
    CPPUNIT_ASSERT(lfb->m_datacenter == "D3");
    CPPUNIT_ASSERT_EQUAL(118, (int)lfb->m_load);
    CPPUNIT_ASSERT_EQUAL(500, (int)lfb->m_target);
    CPPUNIT_ASSERT_EQUAL(2000, (int)lfb->m_max);
