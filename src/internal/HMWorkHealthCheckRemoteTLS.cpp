...
bool
HMWorkHealthCheckRemoteTLS::getHostResult(HMState& state, const string& remoteHost, HMDataCheckResult& remote)
{
    ...
    HMSocketUtilTCPS socketAPI(state.m_ctx, remote.m_address, remote.m_port, tv, m_hostCheck.getSourceAddress(), m_hostCheck.getTOSValue());
    ...
    if (socketAPI.getHostResults(m_hostname, m_ipAddress, m_hostCheck, tv_checkinfo, m_hostResults) == HM_SOCK_DATA_OK)
    {
        ...
        HMLog(HM_LOG_DEBUG,
                "[TLSREMOTECHECK] Trying to get Load feedback Info from %s(%s) for host %s (%s)",
        ...
        {
            HMLog(HM_LOG_DEBUG,
                    "[TLSREMOTECHECK] Obtained Load feedback Info from %s (%s) for host %s (%s)",
                    remoteHost.c_str(), remote.m_address.toString().c_str(), m_hostname.c_str(),
                    m_ipAddress.toString().c_str());
            return true;
        }
        HMLog(HM_LOG_DEBUG,
                "[TLSREMOTECHECK] Failed to get Load feedback Info from %s (%s) for host %s (%s)",
                remoteHost.c_str(), remote.m_address.toString().c_str(), m_hostname.c_str(),
                m_ipAddress.toString().c_str());
    }
    return false;
...
bool
HMWorkHealthCheckRemoteTLS::remoteCheck(HMState& state, HMDataHostGroup& dataHostGroup)
{
    ...
    {
        HMLog(HM_LOG_DEBUG,
                "[TLSREMOTECHECK] Trying to fetch results for %s(%s) from host %s",
        ...
        {
            HMLog(HM_LOG_DEBUG,
                    "[TLSREMOTECHECK] Trying to fetch results for %s (%s) from host %s (%s)",
                    m_hostname.c_str(), m_ipAddress.toString().c_str(),
            ...
            if (state.m_checkList.getCheckResult(header, result))
            {
                ...
                {
                    HMLog(HM_LOG_DEBUG,
                            "[TLSREMOTECHECK] Failed to get results from %s (%s) for host %s (%s)",
                ...
                {
                    HMLog(HM_LOG_DEBUG,
                            "[TLSREMOTECHECK] Obtained results from %s (%s) for host %s (%s)",
                            host->c_str(), result.m_address.toString().c_str(),
                            m_hostname.c_str(), m_ipAddress.toString().c_str());
                    return true;
                }
            ...
            {
                HMLog(HM_LOG_DEBUG,
                        "[TLSREMOTECHECK] Failed to get results for remote host %s (%s)",
                        host->c_str(), result.m_address.toString().c_str());
                continue;
            }
        }
...
HM_WORK_STATUS
HMWorkHealthCheckRemoteTLS::healthCheck()
{
    if (!m_hostCheck.getRemoteCheck().empty())
    {
        ...
        if (hostGroupIt != currentState->m_hostGroups.end())
        {
            ...
            HMLog(HM_LOG_DEBUG3, "[TLSREMOTECHECK] Remote check for %s",
            ...
            {
                HMLog(HM_LOG_DEBUG,
                        "[TLSREMOTECHECK] Obtained results for host %s (%s)",
                        m_hostname.c_str(), m_ipAddress.toString().c_str());
        ...
        {
            HMLog(HM_LOG_DEBUG,
                    "[TLSREMOTECHECK] Host group missing for remote check of %s (%s) for hostgroup %s",
                    m_hostname.c_str(), m_ipAddress.toString().c_str(),
    ...
    {
        HMLog(HM_LOG_ERROR,
                "[TLSREMOTECHECK] Invalid Check type for TCPCheckClient %s",
                printCheckType(m_hostCheck.getCheckType()).c_str());
    }
