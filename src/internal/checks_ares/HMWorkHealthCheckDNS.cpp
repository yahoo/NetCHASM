...
HMWorkHealthCheckDNS::healthCheck()
{
    if (m_hostCheck.getCheckType() == HM_CHECK_DNSVC || m_hostCheck.getCheckType() == HM_CHECK_DNS)
    {
        ...
        else
        {
            ...
            if (m_hostCheck.getSourceAddress().getType() == AF_INET)
            {
                ares_set_local_ip4(channel, m_hostCheck.getSourceAddress().addr4());
            }
            else if(m_hostCheck.getSourceAddress().getType() == AF_INET6)
            {
                ares_set_local_ip6(channel, (const unsigned char*)m_hostCheck.getSourceAddress().addr6().__in6_u.__u6_addr32);
            }
