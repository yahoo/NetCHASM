...
HM_WORK_STATUS
HMWorkHealthCheckTCPS::healthCheck()
{
    if(m_hostCheck.getCheckType() == HM_CHECK_TCPS)
    {
        shared_ptr<HMState> currentState;
        m_stateManager->updateState(currentState);

        HMLog(HM_LOG_DEBUG3, "[TLSCHECK] Raw Socket check TLS for %s",
        ...
        case HM_REASON_SUCCESS:
            ...
            HMLog(HM_LOG_DEBUG3,
                    "[TLSCHECK] Connection successful to host:%s(%s), port:%hu",
                    m_hostname.c_str(), m_ipAddress.toString().c_str(), m_hostCheck.getPort());
            if (checkInfo.empty())
            {
                HMLog(HM_LOG_DEBUG3,
            ...
            else
            {
                ...
                m_response = HM_RESPONSE_FAILED;
                HMLog(HM_LOG_DEBUG3,
                        "[TLSCHECK] checkInfo is present and Ready to read");
                ...
                case HM_REASON_SUCCESS:
                    ...
                    {
                        HMLog(HM_LOG_DEBUG3,
                                "[TLSCHECK] Health check successful for host:%s(%s), port:%hu, checkInfo:%s",
                                m_hostname.c_str(), m_ipAddress.toString().c_str(), m_hostCheck.getPort(),
                    ...
                    {
                        HMLog(HM_LOG_DEBUG3,
                                "[TLSCHECK] tls response for %s(%s) did not match CheckInfo %s",
    ...
    {
        HMLog(HM_LOG_ERROR, "[TLSCHECK] Invalid Check type for TLSCheck %s",
                printCheckType(m_hostCheck.getCheckType()).c_str());
    }
