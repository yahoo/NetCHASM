...
void
HMEventLoopLibEvent::addDNSTimeout(const string& hostname, bool ipv6, HMTimeStamp timeStamp)
{
    DNSTimeout* data = new DNSTimeout(hostname, ipv6);
...
HMEventLoopLibEvent::handleDNSTimeout(evutil_socket_t fd, short what, void* arg)
{
    ...
    else if (check_state == HM_SCHEDULE_EVENT)
    {
        ...
        event->addDNSTimeout(data->m_hostname, ipv6, nextCheckTimeOut);
    }
