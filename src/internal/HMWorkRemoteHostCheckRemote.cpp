...
bool
HMWorkRemoteHostCheckRemote::getHostResult(HMState& state, const string& remoteHost, HMDataCheckResult& remote)
{
    ...
    if (socketAPI->getHostResults(m_hostname, m_hostCheck, tv_checkinfo, m_hostResults) == HM_SOCK_DATA_OK)
    {
        ...
        HM_SOCK_DATA_STATUS status = socketAPI->getLoadFeedback(m_hostname, m_hostCheck,
                tv_checkinfo, m_auxResults);
        if (status == HM_SOCK_DATA_OK || status == HM_SOCK_DATA_EMPTY)
        {
            HMLog(HM_LOG_DEBUG,
                    "[%s-%s] Obtained Load feedback Info from %s (%s) for hostgroup %s size %llu",
                    printFlowType(m_hostCheck.getFlowType()).c_str(),
                    printRemoteCheckType(m_hostCheck.getRemoteCheckType()).c_str(),
                    remoteHost.c_str(), remote.m_address.toString().c_str(), m_hostname.c_str(), m_hostResults.size());
            return true;
        }
        HMLog(HM_LOG_DEBUG,
...
bool
HMWorkRemoteHostCheckRemote::remoteCheck(HMState& state, HMDataHostGroup& dataHostGroup)
{
    ...
    for(auto host = dataHostGroup.getHostList()->begin(); host != dataHostGroup.getHostList()->end(); ++host)
    {
        ...
        for (const HMIPAddress address : addresses)
        {
            ...
            if (state.m_checkList.getCheckResult(header, result))
            {
                ...
                {
                    HMLog(HM_LOG_DEBUG,
                            "[%s-%s] Failed to get results from %s for host %s",
                            printFlowType(m_hostCheck.getFlowType()).c_str(),
                ...
                {
                    HMLog(HM_LOG_DEBUG,
                            "[%s-%s] Obtained results from %s (%s) for host %s size %llu",
                            printFlowType(m_hostCheck.getFlowType()).c_str(),
                            printRemoteCheckType(m_hostCheck.getRemoteCheckType()).c_str(),
                            host->c_str(), result.m_address.toString().c_str(),
                            m_hostname.c_str(), m_hostResults.size());
                    return true;
                }
            ...
            {
                HMLog(HM_LOG_DEBUG,
                        "[%s-%s] Failed to get results for %s from remote host %s (%s)",
                        printFlowType(m_hostCheck.getFlowType()).c_str(),
                        printRemoteCheckType(m_hostCheck.getRemoteCheckType()).c_str(),
                        m_hostname.c_str(),
                        host->c_str(), result.m_address.toString().c_str());
...
void
HMWorkRemoteHostCheckRemote::init(HMWorkState& state)
{
    (void)state;
}

HM_WORK_STATUS
HMWorkRemoteHostCheckRemote::remoteLookup()
{
    ...
    if (!m_hostCheck.getRemoteCheck().empty())
    {
        ...
        if (hostGroupIt != currentState->m_hostGroups.end())
        {
            ...
            {
                HMLog(HM_LOG_DEBUG,
                        "[%s-%s] Obtained results for host %s",
                        printFlowType(m_hostCheck.getFlowType()).c_str(),
                        printRemoteCheckType(m_hostCheck.getRemoteCheckType()).c_str(),
                        m_hostname.c_str());
        ...
        {
            HMLog(HM_LOG_DEBUG,
                    "[%s-%s] Host group missing for remote check of %s (%s) for host %s",
                    printFlowType(m_hostCheck.getFlowType()).c_str(),
                    printRemoteCheckType(m_hostCheck.getRemoteCheckType()).c_str(),
